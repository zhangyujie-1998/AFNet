...
def wrap_class(cls, **kwargs):
  ...
  class NumpyOpWrapper(torch.autograd.Function):
    ...
    def forward(ctx, values: torch.Tensor):
      ...
      ctx.numpy_obj = obj
      ctx.device = values.get_device()
      return torch.from_numpy(obj.compute()).to(ctx.device)

    @staticmethod
    def backward(ctx, grad_output):
      return torch.from_numpy(ctx.numpy_obj.vjp(grad_output.cpu().numpy())).to(ctx.device)
